// Content Script for Pexels
// Extracts metadata and registers downloads

(function() {
  'use strict';

  const SITE_NAME = 'Pexels';
  ...
  function extractCategory() {
    ...
    if (path.includes('/photos') || path.includes('/photo/')) {
      return 'photo';
    }
    return null;
  }
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target && isDownloadElement(target)) {
        ...
        if (href) {
          ...
          console.log('[Stockpile] Registered Pexels download:', href, metadata);
        }
      }
    }, true);
  }
  ...
  function init() {
    console.log('[Stockpile] Pexels content script loaded');
