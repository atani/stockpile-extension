// Content Script for Pond5
// Extracts metadata and registers downloads

(function() {
  'use strict';

  const SITE_NAME = 'Pond5';

  function extractCategory() {
    const path = window.location.pathname.toLowerCase();
    if (path.includes('/stock-video-footage/') || path.includes('/footage/')) return 'video';
    if (path.includes('/stock-music/') || path.includes('/music/')) return 'music';
    if (path.includes('/sound-effect/') || path.includes('/sfx/')) return 'sfx';
    if (path.includes('/stock-photo/') || path.includes('/photo/')) return 'photo';
    if (path.includes('/after-effects/')) return 'ae';
    return 'video';
  }
  ...
  function mapCategory(rawCategory) {
    ...
    if (category.includes('ae') || category.includes('after')) return 'AE_Template';
    return 'Video';
  }
  ...
  function isDownloadUrl(url) {
    ...
    const extensions = ['.mp4', '.mov', '.webm', '.zip', '.jpg', '.jpeg', '.png', '.wav', '.mp3', '.aep'];
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target && isDownloadElement(target)) {
        ...
        if (href) {
          ...
          console.log('[Stockpile] Registered Pond5 download:', href, metadata);
        }
      }
    }, true);
  }
  ...
  function init() {
    console.log('[Stockpile] Pond5 content script loaded');
