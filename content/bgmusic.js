// Content Script for BGMusic
// Extracts metadata and registers downloads

(function() {
  'use strict';

  const SITE_NAME = 'BGMusic';
  ...
  function extractCategory() {
    ...
    if (url.includes('/freejingle/') || url.includes('jingle')) {
      return 'jingle';
    }

    return 'bgm'; // Default - BGMusic is primarily BGM
  }
  ...
  function extractTags() {
    ...
    const tagLinks = document.querySelectorAll('a[href*="/tag/"], a[rel="tag"], .tag-link, .post-tag');
    tagLinks.forEach(el => {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target && isDownloadElement(target)) {
        ...
        if (href) {
          ...
          console.log('[Stockpile] Registered BGMusic download:', href, metadata);
        }
      }
    }, true);
  }
  ...
  // Initialize
  function init() {
    console.log('[Stockpile] BGMusic content script loaded');
