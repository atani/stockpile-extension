// Content Script for FootageCrate
// Extracts metadata and registers downloads

(function() {
  'use strict';

  const SITE_NAME = 'FootageCrate';

  function extractCategory() {
    const path = window.location.pathname.toLowerCase();
    if (path.includes('/vfx/')) return 'vfx';
    if (path.includes('/free-video-footage/')) return 'video';
    if (path.includes('/sound-effects/')) return 'sfx';
  ...
  function isDownloadUrl(url) {
    ...
    const extensions = ['.mp4', '.mov', '.webm', '.zip', '.png', '.wav', '.mp3'];
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target && isDownloadElement(target)) {
        ...
        if (href) {
          ...
          console.log('[Stockpile] Registered FootageCrate download:', href, metadata);
        }
      }
    }, true);
  }
  ...
  function init() {
    console.log('[Stockpile] FootageCrate content script loaded');
