// Content Script for Pixabay
// Extracts metadata and registers downloads

(function() {
  'use strict';

  const SITE_NAME = 'Pixabay';

  function extractCategory() {
    const path = window.location.pathname.toLowerCase();
    if (path.includes('/videos') || path.includes('/video/')) {
      return 'video';
    }
    if (path.includes('/photos') || path.includes('/photo/') || path.includes('/images')) {
      return 'photo';
    }
    return null;
  }
  ...
  function isDownloadUrl(url) {
    ...
    const extensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.mp4', '.mov', '.webm', '.zip'];
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target && isDownloadElement(target)) {
        ...
        if (href) {
          ...
          console.log('[Stockpile] Registered Pixabay download:', href, metadata);
        }
      }
    }, true);
  }
  ...
  function init() {
    console.log('[Stockpile] Pixabay content script loaded');
